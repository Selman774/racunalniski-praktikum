...
% Vsaka od vrstic v okolju `enumerate` je en element naštevanja.
...
\begin{enumerate}
   ...
   \item Nobena skup sme imeti več predavanj hkrati.
...
% Spodnja vrstica ne začne novega odstavka, zato ne bi smela biti zamaknjena.
% To popravite z ukazom `\noindent` ali pa tako, da odstranite prazno vrstico.
% (Nekolko lepše izgleda, če v prazno vrstico vstavite % za komentar.)
...
omejitve so bile kršene ter kolikokrat se kršitev pojavi. S \emph{k}${_n}$ označimo število
kršitev omejitve \emph{n}, z \emph{u}${_n}$ pa njeno utež. Z naslednjo formulo izračunamo
kakovost urnika:
\begin{center}
   \emph{k}${_1}$ $\cdot$ \emph{u}${_1}$ + \emph{k}${_2}$ $\cdot$ \emph{u}${_2}$ + . . . + \emph{k}${_{n-1}}$ $\cdot$ \emph{u}${_{n-1}}$ + \emph{k}${_n}$ $\cdot$ \emph{u}${_n}$
\end{center}
...
\begin{enumerate}
   \item Z zanko se sprehodimo po vektorju vseh predmetov. Predmet na \emph{i}-tem koraku označimo s predmet$_{\emph{i}}$.
   \item Naslednja vgnezdena zanka nas popelje po vseh predavalnicah. Predavalnico na \emph{j}-tem koraku označimo s predavalnica$_{\emph{j}}$.
   \item  Za vse proste termine predavalnice$_{\emph{j}}$ izračunamo ceno vstavljanja srečanja$_{\emph{i}}$, ki ga sestavlja predmet$_{\emph{i}}$, profesor, ki ta predmet predava, ter skupina, kateri je predmet namenjen.
   \item Če je trenutna cena boljša od najboljše dosedanje, potem najboljša dosedanja dobi vrednost trenutne. Hkrati si zapomnimo, za kateri termin smo izračunali ceno vstavljanja, da bomo predmet$_{\emph{i}}$ na koncu vstavili v termin z najboljšo ceno vstavljanja.
   \item Ko preverimo vse proste termine vseh predavalnic, vstavimo srečanje$_{\emph{i}}$ v termin z najboljšo ceno.
...
Naj bo \emph{n}=100 ter \emph{s}${_1}$ sprememba urnika, ki nas je pripeljala v lokalni maksimum. To 
spremembo (npr. zamenjava srečanja$_{\emph{i}}$ in srečanja$_{\emph{j}}$) sedaj shranimo v
seznam tabujev. V naslednjem koraku izvedemo spremembo \emph{s}${_2}$, ki se prav tako shrani v
...
se sprememba \emph{s}${_1}$, ki vodi do lokalnega maksimuma, šele tedaj izbriše iz seznama
...
simbiozo opisanih metod. Nič ne bi bilo narobe, če bi za iskanje najboljšegka posamično
uporabili katero izmed tehnik - mogoče bi katero izmed teh postopanj dalo celo boljši rezultat.

\subsection{Zaključek} % podr
